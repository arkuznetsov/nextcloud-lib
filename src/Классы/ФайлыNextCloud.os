...
Процедура Удалить(Знач ПутьКФайлу, Знач ОшибкаЕслиНеСуществует = Ложь) Экспорт
	...
	Если РезультатЗапроса.КодСостояния = КодыСостояния().НеНайдено Тогда
		...
		КонецЕсли;
	ИначеЕсли НЕ РезультатЗапроса.КодСостояния = КодыСостояния().НетСодержимого Тогда
...
Процедура СоздатьКаталог(Знач ПутьККаталогу, Знач ОшибкаЕслиСуществует = Ложь) Экспорт
	...
	Если РезультатЗапроса.КодСостояния = КодыСостояния().МетодНеПоддерживается Тогда
		...
		Иначе
			Лог.Предупреждение(ТекстОшибки);
		КонецЕсли;
	ИначеЕсли НЕ РезультатЗапроса.КодСостояния = КодыСостояния().Создано Тогда
